...
def get_argparser():
    ...
    parser.add_argument(
        "--train_model_type", type=str, choices=["Unet", "MedSam", "CEmbSam"], default="CEmb-Sam",
...
class TrainMedSam:
    ...
    def evaluate(self, model, val_loader, desc="Validating") -> float:
        ...
        for image, mask, bbox, _ in progress_bar:
    ...
    def early_stopping(
        ...
        if (
            ...
            self.save_model(model, "medsam_finetune_final.pth")
            return True
        return False
...
class TrainCEmbSam:
    ...
    def evaluate(self, model, val_loader, desc="Validating") -> float:
        ...
        for image, mask, bbox, wcode in progress_bar:
            ...
            ###
            image_embeddings = model.con_block(
...
class TrainUnet:
    ...
    def evaluate(self, model, val_loader, desc="Validating") -> float:
        ...
        for image, mask, bbox, wcode, pth in progress_bar:
            image = image.to(self.device)
            mask = mask.to(self.device)
            
            mask_predictions = model(image)
    ...
    def save_model(self, model, model_name=None):
        date_postfix = datetime.now().strftime("%Y-%m-%d-%H-%S")
        if model_name is None:
            model_name = f"unet_finetune_{date_postfix}.pth"
        save_path = os.path.join(
            self.save_pth,
            "finetune_weights"
        )
    ...
    def early_stopping(
        ...
        if (
            ...
            self.save_model(model, "unet_finetune_final.pth")
            return True
        return False
...
def send_result(args, result, start_time):
    ...
    """.format(
        ...
        table = tabulate(Rsts, headers='firstrow', tablefmt='html'),
        time = "Time elapsed: " + str(datetime.now() - start_time)
    )
    email.Email(
        subject = "[MedSam]" if args.train_medsam else "[UltraSam]",
        msg = html
    ).send()
...
if __name__ == "__main__":
    ...
    try:
        is_error = False
        metric = main(args)
        send_result(args, metric[args.train_model_type]["metric"], total_time)
