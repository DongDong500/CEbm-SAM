...
class UltraSamDataset(Dataset):
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        # read dataframe row
        # column = [`study`, `region`, `subdomain`, `image`, `mask`, `Wcode`]
        row = self.df.iloc[idx]
        # If the `image_dir` attribute is set, the path will be relative to that directory.
        # Otherwise, the path will be the value of the `row[self.image_col]` attribute.
        image_file = (
            os.path.join(self.image_dir, row['study'], row['subdomain'], row[self.image_col])
            if row['study'] == "BUSI_with_GT"
            else os.path.join(self.image_dir, row['study'], row['region'], row['subdomain'], row[self.image_col])
        )
        mask_file = (
            os.path.join(self.mask_dir, row['study'], row['subdomain'], row[self.mask_col])
            if row['study'] == "BUSI_with_GT"
            else os.path.join(self.mask_dir, row['study'], row['region'], row['subdomain'], row[self.mask_col])
        )
    ...
    def _get_bbox(self, mask: torch.Tensor) -> torch.Tensor:
        ...
        if len(y_indices) > 0 or len(x_indices) > 0:
            ...
            x_min, y_min = (
                torch.tensor(self.image_size[0]/2, dtype=torch.int), 
                torch.tensor(self.image_size[1]/2, dtype=torch.int)
            )
            x_max, y_max = (
                torch.tensor(self.image_size[0]/2, dtype=torch.int), 
                torch.tensor(self.image_size[1]/2, dtype=torch.int)
...
class SamDataset(Dataset):
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        image, mask, bbox, coor, clbl, wcode = self._preprocess(image_data, mask_data, wcode)
    ...
    def _preprocess(
        ...
        bbox, coor, clbl = self._get_bbox(mask)

        return image, mask, bbox, coor, clbl, wcode
    ...
    def _get_bbox(self, mask: torch.Tensor) -> torch.Tensor:
        ...
        c = [[torch.median(x_indices), torch.median(y_indices)]]
        cl = [1]
        
        return torch.tensor([x_min, y_min, x_max, y_max]), torch.tensor(c, dtype=torch.int32), torch.tensor(cl, dtype=torch.int32)
